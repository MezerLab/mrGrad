function region_axis_visualize(varargin)

error(['mrGrad v2.0: The function region_axis_visualize() is no longer supported.', newline, ...
       'Please use mrgrad_axis_visualize() instead.']);
...
% 
% [found, subplotInfo, varargin] = argParse(varargin, 'subplotInfo');
% if ~found; subplotInfo = []; end
% 
% if fig_ax
%     fig_color = 'w';
% else
...
%             subplotInfo = string(subplotInfo);
%             subplotInfo(~isfield(RG{1},subplotInfo)) = [];
%             ttl = arrayfun(@(x) RG{1}.(x){ii}, subplotInfo,'un',0);
%             ttl = strjoin(ttl," ");
%             title(ttl);
%             hold on
%         end
