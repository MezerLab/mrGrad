function tbl = mrGrad_output2table(RG,Axis)
% RG    mrGrad output with one ROI (left,right) and n groups
% pc    one axis (1/2/3)
...
for gg = 1:size(RG,1)
    for rr = 1:size(RG,2)
        rg = RG{gg,rr};
        nsegs = size(rg.Y{Axis},1);
        nsubs = size(rg.Y{Axis},2);
        for ii = 1:nsubs
            t = table;
            ...
            t.Axis = repmat(Axis,nsegs,1);
%             t.hemisphere_id = double(startsWith(t.RoiName,'Right'));
            t.Position = (1:nsegs)';
            t.Response = rg.Y{Axis}(:,ii);
            tbl = [tbl;t];
        end
    end
end
